...
TKINFO ctkfileIn32;
L2BCMB_SWATHS swath;
L2BCMB_SWATHS swathx;
L2BCMBT_SWATHS swath_t;
                       ...
                       float *sfcPrecipStd, float *piaOut)
{
  ...
  for(i=0;i<49;i++)
    {
      dprswath.FS.Latitude[i]=lat[i];
      dprswath.FS.Longitude[i]=lon[i];
      if(dprswath.FS.Longitude[i]>180)
	dprswath.FS.Longitude[i]-=360;
      dprswath.FS.SLV.precipRateNearSurface[i]=sfcPrecip[i];
      dprswath.FS.SRT.pathAtten[i][0]=piaOut[i];
...
void copyrrates132_(float *rrate, float *rratestd, int *i)
{
  ...
  for(k=0;k<nbins;k++)
    {
      swath.KuGMI.precipTotRate[*i][k]=rrate[k];
      swath.KuGMI.precipTotRateSigma[*i][k]=rratestd[k];
    }
}
...
void copytruerrate32_(float *rrate, int *i)
{
  ...
  for(k=0;k<nbins;k++)
    {
      dprswath.FS.SLV.precipRate[*i][2*k]=(int)(rrate[k]*100);
      dprswath.FS.SLV.precipRate[*i][2*k+1]=(int)(rrate[k]*100);
    }
}
...
void copypwcs132_(float *rrate, float *rratestd, int *i)
{
  ...
  for(k=0;k<nbins;k++)
    {
      swath.KuGMI.precipTotWaterCont[*i][k]=rrate[k];
      swath.KuGMI.precipTotWaterContSigma[*i][k]=rratestd[k];
    }
}
...
void copyd0s132_(float *dm, int *i)
{
  ...
  for(k=0;k<nbins;k++)
    {
      //swath.KuGMI.precipTotPSDparamHigh[*i][k]=dm[k];
    }
}
...
void copyzckus132_(float *zc, int *i)
{
  ...
  extern L2BCMB_SWATHS swath;
   ...
   for(k=0;k<nbins;k++)
    {
      swath.KuGMI.correctedReflectFactor[*i][k]=zc[k];
    }
}
...
void copynodess132_(int *node, int *i)
{
  ...
  for(k=0;k<5;k++)
    {
      swath.KuGMI.phaseBinNodes[*i][k]=node[k];
    }
}
...
void copyscantime32_(int *i)
{
 ...
 swath.KuGMI.ScanTime.DayOfMonth=DayOfMonth[*i];
 swath.KuGMI.ScanTime.DayOfYear=DayOfYear[*i];
 swath.KuGMI.ScanTime.Hour=Hour[*i];
 swath.KuGMI.ScanTime.MilliSecond=MilliSecond[*i];
 swath.KuGMI.ScanTime.Minute=Minute[*i];
 swath.KuGMI.ScanTime.Month=Month[*i];
 swath.KuGMI.ScanTime.Second=Second[*i];
 swath.KuGMI.ScanTime.Year=Year[*i];
}
