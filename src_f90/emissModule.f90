...
!end  WSO  11/7/13
   ...
   if(snowicecover .eq. 2 .or. snowicecover .eq. 3) then
    emissv(1) = LUT%emis_map_snow(1,jj,ii)
    emissv(2) = LUT%emis_map_snow(3,jj,ii)
    emissv(3) = LUT%emis_map_snow(5,jj,ii)
    emissv(4) = LUT%emis_map_snow(6,jj,ii)
    emissv(5) = LUT%emis_map_snow(8,jj,ii)
    emissv(6) = LUT%emis_map_snow(10,jj,ii)
    emissh(1) = LUT%emis_map_snow(2,jj,ii)
    emissh(2) = LUT%emis_map_snow(4,jj,ii)
    emissh(4) = LUT%emis_map_snow(7,jj,ii)
    emissh(5) = LUT%emis_map_snow(9,jj,ii)
    emissh(6) = LUT%emis_map_snow(11,jj,ii)
!begin  SJM 2/14/17 read emissivity std deviations
    emissv_std(1) = LUT%estd_map_snow(1,jj,ii)
    emissv_std(2) = LUT%estd_map_snow(3,jj,ii)
    emissv_std(3) = LUT%estd_map_snow(5,jj,ii)
    emissv_std(4) = LUT%estd_map_snow(6,jj,ii)
    emissv_std(5) = LUT%estd_map_snow(8,jj,ii)
    emissv_std(6) = LUT%estd_map_snow(10,jj,ii)
    emissh_std(1) = LUT%estd_map_snow(2,jj,ii)
    emissh_std(2) = LUT%estd_map_snow(4,jj,ii)
    emissh_std(4) = LUT%estd_map_snow(7,jj,ii)
    emissh_std(5) = LUT%estd_map_snow(9,jj,ii)
    emissh_std(6) = LUT%estd_map_snow(11,jj,ii)
!end  SJM 2/14/17
  else if(snowicecover .eq. 0 .or. snowicecover .eq. 1) then
    emissv(1) = LUT%emis_map_bare(1,jj,ii)
    emissv(2) = LUT%emis_map_bare(3,jj,ii)
    emissv(3) = LUT%emis_map_bare(5,jj,ii)
    emissv(4) = LUT%emis_map_bare(6,jj,ii)
    emissv(5) = LUT%emis_map_bare(7,jj,ii)
    emissv(6) = LUT%emis_map_bare(10,jj,ii)
    emissh(1) = LUT%emis_map_bare(2,jj,ii)
    emissh(2) = LUT%emis_map_bare(4,jj,ii)
    emissh(4) = LUT%emis_map_bare(7,jj,ii)
    emissh(5) = LUT%emis_map_bare(9,jj,ii)
    emissh(6) = LUT%emis_map_bare(11,jj,ii)
!begin  SJM 2/14/17 read emissivity std deviations
    emissv_std(1) = LUT%estd_map_bare(1,jj,ii)
    emissv_std(2) = LUT%estd_map_bare(3,jj,ii)
    emissv_std(3) = LUT%estd_map_bare(5,jj,ii)
    emissv_std(4) = LUT%estd_map_bare(6,jj,ii)
    emissv_std(5) = LUT%estd_map_bare(8,jj,ii)
    emissv_std(6) = LUT%estd_map_bare(10,jj,ii)
    emissh_std(1) = LUT%estd_map_bare(2,jj,ii)
    emissh_std(2) = LUT%estd_map_bare(4,jj,ii)
    emissh_std(4) = LUT%estd_map_bare(7,jj,ii)
    emissh_std(5) = LUT%estd_map_bare(9,jj,ii)
    emissh_std(6) = LUT%estd_map_bare(11,jj,ii)
!end  SJM 2/14/17
  else
...
!begin  SJM 2/14/17 read emissivity std deviations
    ...
    emissv_std(3) = 0.64*emissv_std(2)+0.36*emissv_std(4)
!end  SJM 2/14/17
  endif
  ...
  if(emissv(3) .le. 0.) emissv(3) = 0.64*emissv(2)+0.36*emissv(4)
