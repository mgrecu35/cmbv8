...
!  SFM  start  09/25/2013
  character(16), parameter :: algorithmVersion = "2BCMB_20220401"
  !  SFM  end  09/25/2013
...
!  SFM  start  01/02/2014
    ...
    INTEGER*4     readdprpflag, readdprpflagx, readdprpflag_t     ! declare function
    !LW 05/04/18
...
!test
    print*, 'in mainfort_t at beginning'
!end test
...
!.Look for Cases of Bad 2Aku
  ...
  if(ialg==2) then
     continue
     st_2aku = readdprpflag_t(jobname, f2AKu(1:i2AKu),ndpr,rlon,rlat, &
             iLandOcean, rPrecipFlag, rSfcPrecip)
!diagnostic
     print*, 'In mainfort_t, st_2aku: ', st_2aku
!end diagnostic
     print*, ndpr
  else
     if(ifs==1) then
...
!.Process Case of Empty 2Aku Granule
  IF (st_2aku .EQ. -3) THEN
      ...
      !CALL write_empty()
      if(ialg==2) then
         CALL write_emptyt() !3/11/22 WSO uncomment empty write for TRMM
      print*, 'In mainfort_t; wrote empty granule here'
      else
         CALL write_empty()
      endif
...
!begin 2/9/22 WSO assign status code to st_TMI2
     st_TMI2 = st_2
     print*, "in mainfort_t, st_2: ", st_2, "  st_TMI2: ", st_TMI2
!end 2/9/22
