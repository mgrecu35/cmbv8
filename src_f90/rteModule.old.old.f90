...
subroutine getscattH(binNodes,pRate,z13,emiss,envNode,pType,&
  ...
  do k=11, min(binNodes(i,j,3)-1,clutFree(i,j))
     ...
     if (pType(i1,j)>0 .and. z13(i1,j,k+1)>10 ) then
        ...
        !f=0.0
!diagnostic
        if(isnan(nw3d(i1, j, k + 1))) then
          print*, "i: ", i, "  i1: ", i1, "  j: ", j, "  k: ", k, "  nw3d: ", nw3d(i1, j, k + 1)
          print*, "binNodes(i, j, 3)-1: ", binNodes(i, j, 3) - 1, "  clutFree(i,j): ", clutFree(i, j), "  idir: ", idir
          do kk = 11, min(binNodes(i,j,3)-1,clutFree(i,j))
            i1=i+int((88-kk)*dx/dx0+0.5)*idir
            if(i1<1) i1=1
            if(i1>nscans) i1=nscans
            print*, "i: ", i, "  i1: ", i1, "  j: ", j, "  k: ", kk, "  nw3d: ", nw3d(i1, j, kk + 1)
          end do
          stop
        endif
!end diagnostic
        ...
        if(n0w1<-3.5) n0w1=-3.5
        if(isnan(n0w1)) then
          print*, "n0w1 before getsnowp_z2: ", n0w1 
        endif
        niter_n0w1 = 0
10      continue
        ...
        if( isnan(kext(1)) .or. maxval(kext)>1.9e5) then
           ...
           print*, salb
           print*, asym
           if(niter_n0w1 > 20) stop
           niter_n0w1 = niter_n0w1 + 1
          !stop
           if(n0w1>1) then 
              n0w1=1
           else
...
!diagnostic
  do k = 1, nlev
    if(nw3d(i, j, k) .ne. nw3d(i, j, k)) then
      print*, "nan in slant1d at i: ", i, "  j:, ", j, "  k: ", k, "  nw3d: ", nw3d(i, j, k)
      stop
    endif
  enddo
!end diagnostic
